...
def evaluate_model(model, X_test, y_test, category_names):
    ...
    for col in y_test.columns.unique():
        total_f1_score.append(f1_score(y_test[col].values, y_pred[col].values,
                              average='weighted'))
        total_recall_score.append(recall_score(y_test[col].values,
                                  y_pred[col].values, average='weighted'))
        total_precision_score.append(precision_score(y_test[col].values,
                                     y_pred[col].values, average='weighted'))
